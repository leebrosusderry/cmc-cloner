...
- Return label: A prepaid return shipping label is included in the package and also re-sent by email with the RMA. {{ten_doanh_nghiep}} covers 100% of return shipping — the customer does NOT pay for return shipping under any circumstance.
...
2. Return Eligibility — literal product condition required: "Only new products — unopened, in original packaging, unused, with all tags attached". Returns cover both defective and non-defective products. Items showing signs of use or damage are not eligible.
3. Return Window — literal "{{return_window}} from the date of delivery", framed as a hard cutoff. The duration {{return_window}} (e.g. "30 days", "100 nights") MUST appear verbatim — do NOT round or paraphrase.
...
5. Exchange Option — affirm literally: "Yes, exchanges are accepted within the {{return_window}} window from delivery for new, unused products". Do NOT reduce exchanges to "handled as a return followed by a new order".
6. Return Method, Label & RMA — MUST state literally: return method is "By mail only"; a prepaid return shipping label is included in the package (and re-sent with the RMA); {{ten_doanh_nghiep}} covers 100% of return shipping — the customer does NOT pay for return shipping; there is "No restocking fee"; the RMA number and prepaid label are issued {{rma_issuance_time}}.
7. How to Initiate a Return or Exchange — numbered step-by-step process: (a) email {{email_web}} with the order ID and reason within the {{return_window}} window from delivery; (b) support replies {{response_time}}; (c) receive the RMA number and prepaid return label {{rma_issuance_time}}; (d) pack the item in its original packaging and ship back via the provided prepaid label by mail; (e) once received and inspected, a refund or exchange is processed.
...
- Every generated page MUST literally contain: "United States", "USD", "{{return_window}} from the date of delivery", "Only new products", "By mail only", "prepaid return shipping label", "the customer does NOT pay for return shipping" (or wording that unambiguously says the store covers return shipping), "No restocking fee", "original payment method", and "{{refund_processing_time}}" (in the context of refund processing). These are non-negotiable.
...
- {{ELIGIBILITY}} — section 2 body (Return Eligibility). Include "Only new products — unopened, in original packaging, unused, with all tags attached" literally, and state that both defective and non-defective products are accepted.
...
- {{EXCHANGE}} — section 5 body (Exchange Option). Affirm "Yes, exchanges are accepted within the {{return_window}} window from delivery" literally.
- {{RETURN_SHIPPING}} — section 6 body (Return Method, Label & RMA). MUST include literally: "By mail only", "prepaid return shipping label included in the package", the customer does NOT pay return shipping, "No restocking fee", RMA issued {{rma_issuance_time}}.
